...
class Strip:
    def __init__(self, led_port, num):
        # led_port : port for KUBOT LED ttyUSB
        # baudrate : baud rate
...
def main(argv):
    prot = rospy.get_param("~prot", '/dev/ttyUSB0')
    num = rospy.get_param("~num", 10)
    mode = rospy.get_param("~mode", 5)
    baudrate = rospy.get_param("~baudrate", 115200)

    try:
        opts, args = getopt.getopt(argv, "hp:n:m:", ["port=", "num=", "mode=", "baudrate="])
    except getopt.GetoptError:
        print(
            "python led_control.py -p <LED ttyUSB> -n <Number of LED units> -m <LED mode> -b <LED baudrate>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
